...
var (
  ...
  buildID int32 = int32(time.Now().Unix())
  srcDir = "src/gopkg.in/ganggo/ganggo.%s"
...
func main() {
  ...
  if _, err := os.Stat(userConfigPath); os.IsNotExist(err) {
    ...
    var section = "ganggo"
    config, err := config.ReadDefault(fmt.Sprintf(configPath + ".example", version))
  ...
  // Run overseer as update manager
...
func prog(state overseer.State) {
  ...
  err = config.WriteFile(fmt.Sprintf(configPath, version), 0644, "AUTO-GENERATED BY GANGGO WIZARD")
  if err != nil {
    panic(err)
  }

  command := fmt.Sprintf("%s", binFile)
  if runtime.GOOS != "windows" {
    command  = "./" + command
  }
  cmd := exec.Command(command, "-importPath", fmt.Sprintf(
    "gopkg.in/ganggo/ganggo.%s", version,
